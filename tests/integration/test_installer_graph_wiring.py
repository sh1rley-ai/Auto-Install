...
from core.installer import DeployBot
...
def test_install_software_delegates_to_build_graph_and_returns_true_on_success(tmp_path, monkeypatch):
    ...
    installer = DeployBot({"deepseek_api_key": "x", "kimi_api_key": "y"})
...
def test_run_shell_tool_captures_returncode_stdout_stderr():
    installer = DeployBot.__new__(DeployBot)  # skip __init__, avoid constructing real clients
