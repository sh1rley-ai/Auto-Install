...
示例用法:
  ...
  python main.py --show-config           # 显示当前配置
    ...
    if args.max_steps:
        enhanced_config.investigation.max_investigation_steps = args.max_steps
    ...
    try:
        ...
        if args.verbose:
            print(f"\n📋 配置信息:")
            print(f"  - 最大步骤数: {enhanced_config.investigation.max_investigation_steps}")
...
def interactive_config():
    ...
    qwen_key = input("请输入 Qwen (DashScope) API Key (可选，用于摘要与记忆蒸馏): ").strip()

    # Get other settings
    max_steps = input("最大调查步骤数 (默认30): ").strip()
    if not max_steps:
        max_steps = "30"
    ...
    if qwen_key:
        enhanced_config.ai_models.qwen_api_key = qwen_key

    try:
        enhanced_config.investigation.max_investigation_steps = int(max_steps)
    except ValueError:
        print("[WARN] 无效的步骤数，使用默认值30")
...
if __name__ == "__main__":
    ...
    if not enhanced_config.ai_models.deepseek_api_key:
        ...
        if choice in ['y', 'yes', '是']:
            ...
            print("环境变量: DEEPSEEK_API_KEY, DASHSCOPE_API_KEY")
            sys.exit(1)
